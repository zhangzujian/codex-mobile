...
9. If the `Run in terminal` quick-command menu is visible next to the terminal button, confirm its icon is distinct from the terminal toggle icon and choose `npm run dev`
...
- Clicking the terminal button directly opens or hides the drawer; users do not need to open the `Run in terminal` menu first
...
- The quick-command menu sends common project commands such as `npm run dev` into the current PTY, including when the terminal drawer was closed before selecting the command
- The terminal open/hide action remains available from the dedicated terminal button, and is not duplicated inside the `Run in terminal` menu
- The `Run in terminal` menu shows discovered project commands in usage order and scrolls when the list is longer than the visible menu
